...
`contacts.get` is also implemented and accepts only a Host-issued opaque reference from the current Host lifetime. The Host keeps a bounded, ephemeral in-memory reference-to-native-identifier mapping; it is never persisted or sent over IPC. Unknown or stale references fail closed. `contacts.list_limited` remains unimplemented.
