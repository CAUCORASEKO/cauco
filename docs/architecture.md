...
Native Capability Broker v1 is a typed, fail-closed in-process contract in `CaucoHostCore`; no transport is implemented and Core cannot call it yet. It accepts only structured requests with explicit local-Core provenance and a closed allowlist of `contacts.status`, `contacts.search`, `contacts.get`, and `contacts.list_limited`. Only `contacts.status` is implemented against authorization state; the other operations are declared and return `notImplemented`. No contact records are read, permission prompt is issued, or request data persisted. Future transport must authenticate local Core and preserve this contract. Arbitrary methods, raw Objective-C objects, unrestricted shell/native execution, and generic IPC remain prohibited.
