...
Native Capability Broker v1 is a typed, fail-closed contract in `CaucoHostCore`, transported only over the Host-created private owner-only Unix socket. The ephemeral token authenticates the owned Core child; no TCP, HTTP, XPC, or persistent session is used. It accepts only structured requests with explicit local-Core provenance and a closed allowlist of `contacts.status`, `contacts.search`, `contacts.get`, and `contacts.list_limited`. Only `contacts.status` is implemented against authorization state; the other operations are declared and return `notImplemented`. No contact records are read or transported. Arbitrary methods, raw Objective-C objects, unrestricted shell/native execution, and generic IPC remain prohibited.
